package org.sdmx.stores.xml.v2.structure
{
	...
	public class XSKeyFamilyTest extends TestCase
	{
		public function XSKeyFamilyTest(methodName:String=null)
		{
			super(methodName);
		}
		...
		{
			return new TestSuite(XSKeyFamilyTest);
		}
		
		public function testExtractXSKeyFamily():void
		{
		...
		private function handleKeyFamilies(event:SDMXDataEvent):void
		{
			...
			assertEquals("There should be 12 attributes", 12, 
				keyFamily.attributeDescriptor.length);	
			
			//Tests to find XSMeasure
			for each (var dim:Dimension in keyFamily.keyDescriptor) {
				if (dim.conceptIdentity.id == "CURRENCY") {
					assertTrue("Currency is the XSMeasure", 
						dim is MeasureTypeDimension);
				} else {
					assertFalse(dim.conceptIdentity.id +  " should not be a" + 
						" XSMeasure", dim is MeasureTypeDimension);
				}
			}
			
			//Tests to check XSAttachmentLevel of dimensions
			assertEquals("FREQ should be attached at the Group level", 
				XSAttachmentLevel.GROUP, 
				(keyFamily.keyDescriptor.getDimension("FREQ") as Dimension).xsAttachmentLevel);
			assertEquals("CURRENCY_DENOM should be attached at the Section level", 
				XSAttachmentLevel.SECTION, 
				(keyFamily.keyDescriptor.getDimension("CURRENCY_DENOM") as Dimension).xsAttachmentLevel);
			assertEquals("EXR_TYPE should be attached at the Section level", 
				XSAttachmentLevel.SECTION, 
				(keyFamily.keyDescriptor.getDimension("EXR_TYPE") as Dimension).xsAttachmentLevel);
			assertEquals("EXR_SUFFIX should be attached at the Section level", 
				XSAttachmentLevel.SECTION, 
				(keyFamily.keyDescriptor.getDimension("EXR_SUFFIX") as Dimension).xsAttachmentLevel);		
			assertEquals("TIME_PERIOD should be attached at the Group level", 
				XSAttachmentLevel.GROUP, 
				(keyFamily.keyDescriptor.getDimension("TIME_PERIOD") as Dimension).xsAttachmentLevel);			
				
			//Tests to check XSAttachmentLevel of attributes
			assertEquals("TIME_FORMAT should be attached at the Group level", 
				XSAttachmentLevel.GROUP, 
				(keyFamily.attributeDescriptor.getAttribute("TIME_FORMAT") as Attribute).xsAttachmentLevel);
			assertEquals("COVERAGE should be attached at the Section level", 
				XSAttachmentLevel.SECTION, 
				(keyFamily.attributeDescriptor.getAttribute("COVERAGE") as Attribute).xsAttachmentLevel);
			assertEquals("OBS_CONF should be attached at the Observation level", 
				XSAttachmentLevel.XSOBSERVATION, 
				(keyFamily.attributeDescriptor.getAttribute("OBS_CONF") as Attribute).xsAttachmentLevel);			
		}
		
		private var _testXSKF:XML = 
<message:Structure 
